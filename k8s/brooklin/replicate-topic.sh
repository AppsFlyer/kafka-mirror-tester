...
kubectl --context eu-west-1.k8s.local exec -n brooklin $brooklin_pod -- bash -c "\$BROOKLIN_HOME/bin/brooklin-rest-client.sh -o CREATE -u http://localhost:32311/ -n mirror-$topic_name -s \"kafka://$kafka_source_ip:9093/^$topic_name$\" -c kafkaMirroringC -t kafkaTP -m '{\"owner\":\"test-user\",\"system.reuseExistingDestination\":\"false\"}' 2>/dev/null"
