...
zookeeper-shell $(kubectl get node $(kubectl -n kafka-source get po pzoo-source-0 -o jsonpath='{.spec.nodeName}') -o jsonpath='{.status.addresses[?(@.type=="ExternalIP")].address}'):2181 get /brokers/ids/0
