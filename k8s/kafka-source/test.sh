...
until POD=$(kubectl --context us-east-1.k8s.local -n kafka-source get po kafka-source-1 | grep Running)
do
  echo "ZK on source isn't ready yet"
  sleep 5
done
...
kubectl --context us-east-1.k8s.local exec -n kafka-source kafka-source-1 -- bash -c "unset JMX_PORT; /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --from-beginning --topic test_topic_1 --max-messages 1"
